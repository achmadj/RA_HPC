...
def random_split(X, y, train_split, val_split):
  N = X.shape[0]
...
def data_preparation(filename, bs=128, N=20000, L=40):
  ...
  X_train, y_train, X_val, y_val = random_split(X, y, 0.8, 0.2)
...
if __name__ == '__main__':
  NOW = sys.argv[1]
  train_set, train_loader, val_set, val_loader = data_preparation(f'/clusterfs/students/achmadjae/RA/02_deep3D/data/{NOW}_train_40_15000.h5')
  # MCOC
  ...
  plt.scatter(y_true, y_pred, s=10, c=c, cmap='inferno_r')
  plt.colorbar()
  plt.xlabel('True')
  plt.ylabel('Pred')
